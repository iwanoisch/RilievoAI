...
import {AnagraficaSectionForm} from "../../../components/anagrafica-section-form/AnagraficaSectionForm.tsx";

export const AnagraficaTab: FC = () => {
    const {t} = useTranslation();
    const {id: buildingId} = useParams<{id: string}>();
    const {getSectionsForBuilding} = useAnagrafica();
        ...
        <div className="card mt-4 p-0 overflow-hidden">
            <div className="flex flex-col lg:flex-row min-h-[500px]">
                ...
                <nav className="hidden lg:block w-72 shrink-0 border-r border-border-default bg-surface-page/50 p-3 space-y-0.5">
                    {ANAGRAFICA_SECTIONS.map((section) => {
                ...
                <div className="lg:hidden p-3 border-b border-border-default bg-surface-page/50">
                    <select
                        id="anagrafica-section-select"
                        name="anagrafica-section-select"
                ...
                <div className="flex-1 min-w-0 p-4 sm:p-6">
                    {selectedId ? (
                        <AnagraficaSectionForm buildingId={buildingId} sectionId={selectedId}/>
                    ) : (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <h3 className="text-base font-semibold text-text-primary">
                                {t('buildingDetail.anagrafica_title')}
                            </h3>
                            <p className="mt-1 text-sm text-text-muted max-w-sm">
                                {t('buildingDetail.anagrafica_hint')}
                            </p>
                        </div>
                    )}
                </div>
